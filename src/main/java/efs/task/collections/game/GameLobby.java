...
public class GameLobby {
    ...
    public List<Hero> getHeroesFromTown(Town town) {
        List<Hero> DLCHeroList = playableTownsWithHeroesList.get(town);
        if (DLCHeroList != null)
            return DLCHeroList;
        else
    ...
    public Map<Town, List<Hero>> mapHeroesToStartingTowns(List<Town> availableTowns, Set<Hero> availableHeroes) {
        Map<Town, List<Hero>> townHeroListMap = new TreeMap<>(Comparator.comparing(Town::getTownName));
        for (Town t : availableTowns) {
            ...
            townHeroListMap.put(t, HeroList);
        }
